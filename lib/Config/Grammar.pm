...
sub _parse_file($$)
{
    ...
    my $fh;
    my $mode = "<";
    $mode .= ":encoding(UTF-8)" if $self->{'utf8'};
...
=head1 SYNOPSIS
 ...
 my $utf8 = 1; # the second paramter to parse() is optional (default 0)
 my $parser = Config::Grammar->new(\%grammar);
 my $cfg = $parser->parse('app.cfg', $utf8) or die "ERROR: $parser->{err}\n";
...
Config::Grammar is a module to parse configuration files. Files are
+opened in UTF8 mode if the optional second parameter to the parse()
+method is true.
...
=head3 Code
 ...
 my $utf8 = 1; # the second paramter to parse() is optional (default 0)
 my $cfg = $parser->parse('test.cfg', $utf8) or
